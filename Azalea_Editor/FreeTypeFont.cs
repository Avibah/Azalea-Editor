...
namespace Azalea_Editor
{
    ...
    public class FreeTypeFont
    {
        ...
        public void PrintMathMap(string text, float x, float y, float size)
        {
            ...
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++)
                PrintWithCurrentProjection(lines[lineIndex], x, viewporSize.Height - y - _fontSize - _fontSize * lineIndex, size);
            PopProjectionMatrix();
        }
        ...
        public void Print(string text, float x, float y, float size)
        {
            if (string.IsNullOrEmpty(text))
                return;
            string[] lines = text.Split('\n');
            ...
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++)
                PrintWithCurrentProjection(lines[lineIndex], x, y + _fontSize * lineIndex, size);
            //PopProjectionMatrix();
        }

        public void PrintWithCurrentProjection(string text, float x, float y, float size)
        {
            int font = _listBase;
            ...
            GL.PushMatrix();
            // GL.LoadIdentity();
            GL.Translate(x, y + _baseLine * size, 0);
